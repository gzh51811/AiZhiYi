...
class Cate extends Component{
	constructor(){
		super();
		this.state={
			cate:["专馆基地","活动专区","传统工艺","民俗文化","茶艺茶道","特产美食","珠宝首饰","文化创意","个性定制"],
			dataStore:[],
			dataStoreHot:[],
			datalist:[{
				cate:[],
    ...
    render(){
		...
		<div className="categroy-banner top_banner"><a href="javascript;;">
			<img src={require("../img/z141.jpg")}  className="cate_hearder_img"/></a></div>
		<dt>
			<a href="javascript:;">
		...
		</dt>
		{
				this.state.dataStoreHot.map((item)=>{
					return <dd key={item._id}>
						<a href="javascript:;" className="seckill_zone">
							<span><img src={item.store_avatar}/></span>
							<p style={{"fontSize":"12px","whiteSpace":"nowrap", "overflow":"hidden","textOverflow":"ellipsis"}}>{item.store_name}</p>
						</a>
					</dd>
	...
	</dl>):(<dl className="brands-recommend hot-area">
		<div className="categroy-banner top_banner"><a href="javascript;;">
			<img src={this.state.datalist[0].wap_goodsclass_image_info.image_path}  className="cate_hearder_img"/></a></div>
		{
			this.state.datalist[0].cate.map((item)=>{
				return (<>
				<dt>
					<a href="javascript:;" onClick={this.toList(item.gc_name)}>
						<img src="https://www.aizhiyi.com/wap/images/cloud.png?999"/>
						<span className="cat-two">{item.gc_name}</span>
					</a>
				</dt>
				{item.child.map((item1)=>{
						return <dd onClick={this.toList(item1.gc_name)}>
							<a href="javascript:;" className="seckill_zone">
								<span><img src={item1.wap_goodsclass_image_info.wap_advs1.image_path}/></span>
								<p>{item1.gc_name}</p>
							</a>
						</dd>
				})}
