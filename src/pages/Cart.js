...
class Cart extends Component{
    ...
    CheckGooditem(gooditem,e){
        let {goodslist,goods_arr,goods_id_arr,goodsprice,goodsNumber} = this.state;
        goods_arr=[];
        goodsprice=0;
        goodsNumber=0;
    ...
    //UI界面渲染
    render(){
        let {goodsobj,allChecked,goods_id_arr,goodsprice,goodsNumber,fix,headerCart,edit} = this.state;
        return <div>
                ...
                <header className="header-cart" style={headerCart}>
                    ...
                    <div className="header-edit">
                        {
                            edit?<span className="edit_cart" id="edit_btn" onClick={this.handleEdit}>编辑</span>:<span className="edit_cart" id="edit_btn" onClick={this.handleComplete}>完成</span>
                        }
                        
                    </div>
                    <div className="header-r">
                        <a id="header-nav" href="javascript:;"><i className="more"></i><sup></sup></a>
                    </div>
                    </div>
                </header>
                    ...
                    <div>
                        {
                            Object.keys(goodsobj).map(item=>
                                <div className="nctouch-cart-container" key={item}>
                                    <List.Item  style={{borderBottom:"1px solid #e8e8e8",position:"relative"}}>
                                        <span className="store-check">
                                            <Checkbox className="store_checkbox" id="" >{item}</Checkbox>
                                            {/* <Checkbox className="store_checkbox" id=""   onChange={this.CheckItem.bind(this,item)}>{item}</Checkbox> */}
                                        </span>
                                        <span className="freight"><i></i>满88元免运费 !</span>
                                    </List.Item>
                                    {
                                            ...
                                            <List.Item  style={goodsstyle} key={gooditem.gTitle}>
                                                <div className="goods-item">
                                                    <div className="goods-check">
                                                        <Checkbox className="store_checkbox" checked={goods_id_arr.includes(gooditem.goods_id)}  onChange={this.CheckGooditem.bind(this,gooditem)}></Checkbox>    
                                                    </div>
                                                    ...
                                                    <div className="goods-subtotal">
                                                        ...
                                                        <div className="value-box" style={edit?value_box_init:value_box_last}>
                                                            <span className="minus" style={edit?none:{}} onClick={this.goodMinus.bind(this,gooditem)}>
                                                                <a href="javascript:;">&nbsp;</a>
                                                            </span>
                                                            <span>
                                                                <i className="num_mind" style={edit?num_mind:none}>x</i>
                                                                <input type="text" pattern="[0-9]*" style={edit?value_input_init:value_input_last} readOnly className="buy-num buynum" onChange={function(){}} value={gooditem.gNumber}></input>
                                                            </span>
                                                            <span className="add" style={edit?none:{}} onClick={this.goodAdd.bind(this,gooditem)}>
                                                                <a href="javascript:;">&nbsp;</a>
                                                            </span>
                                                        </div>
                                                    </div>
                        ...
                        <div className="nctouch-cart-bottom ">
                            <div className="all-check">
                                <Checkbox className="store_checkbox" checked={allChecked} onChange={this.handelallChecked.bind(this)}><span>全选</span></Checkbox> 
                            </div>
                            <div className="total general">
                                <dl className="total-money">
                                    <dt>合计：</dt>
                                    <dd>¥<em>{goodsprice.toFixed(2)}</em></dd>
                                </dl>
                            </div>
                            <div className="check-out" style={{backgroundColor:(goods_id_arr.length>0?"#d33d3c":"#BBB")}}>
                                <a href="javascript:;">
                                    结算
                                    <i>(&nbsp;{goodsNumber}&nbsp;)</i>
                                </a>
